    ...
    return (
        <div ref={sortRef} className="sort">
            <div className="sort__label">
                ...
                <b>Сортировка по:</b>
                <span onClick={() => setOpenPopup(!openPopup)}>{(selectedSort.name)}</span>
            </div>
            {openPopup && (
                <div className="sort__popup">
                    <ul>
                        {typeSort.map((type, index) => <li key={type.name}
                                                           onClick={() => switchSort(type)}
                                                           className={selectedSort.sortValue === type.sortValue ? 'active' : ''}>{type.name}</li>)}
                    </ul>
                </div>
            )}
        </div>
